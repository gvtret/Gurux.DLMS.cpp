...
int CGXXmlWriter::WriteElementObject(
    ...
    if (uiType != DLMS_DATA_TYPE_NONE && dt != uiType &&
        (uiType != DLMS_DATA_TYPE_STRING || dt == DLMS_DATA_TYPE_OCTET_STRING)) {
...
int SaveActionSchedule(CGXXmlWriter *writer, CGXDLMSActionSchedule *obj) {
    ...
    if ((ret = writer->WriteElementString("LN", obj->GetExecutedScriptLogicalName())) == 0 &&
        ...
        for (std::vector<CGXDateTime>::iterator it = obj->GetExecutionTime().begin();
             it != obj->GetExecutionTime().end(); ++it) {
    ...
    if ((ret = writer->WriteStartElement(name)) == 0) {
        for (std::vector<CGXDLMSDayProfile *>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) == 0 &&
                ...
                for (std::vector<CGXDLMSDayProfileAction *>::iterator d = (*it)->GetDaySchedules().begin();
                     d != (*it)->GetDaySchedules().end(); ++d) {
...
int SaveAssociationLogicalName(CGXXmlWriter *writer, CGXDLMSAssociationLogicalName *obj) {
    ...
    if ((ret = writer->WriteElementString("ClientSAP", obj->GetClientSAP())) == 0 &&
        ...
        (ret = writer->WriteElementString("MaxReceivePduSize", obj->GetXDLMSContextInfo().GetMaxReceivePduSize())) ==
            0 &&
        ...
        (ret = writer->WriteElementString("JointIsoCtt", obj->GetAuthenticationMechanismName().GetJointIsoCtt())) ==
            0 &&
        (ret = writer->WriteElementString("Country", obj->GetAuthenticationMechanismName().GetCountry())) == 0 &&
        (ret = writer->WriteElementString("CountryName", obj->GetAuthenticationMechanismName().GetCountryName())) ==
            0 &&
        (ret = writer->WriteElementString(
             "IdentifiedOrganization", obj->GetAuthenticationMechanismName().GetIdentifiedOrganization()
         )) == 0 &&
        (ret = writer->WriteElementString("DlmsUA", obj->GetAuthenticationMechanismName().GetDlmsUA())) == 0 &&
        (ret = writer->WriteElementString(
             "AuthenticationMechanismName", obj->GetAuthenticationMechanismName().GetAuthenticationMechanismName()
         )) == 0 &&
        (ret = writer->WriteElementString("MechanismId", obj->GetAuthenticationMechanismName().GetMechanismId())) ==
            0 &&
        (ret = writer->WriteEndElement()) == 0 &&
...
int SaveAutoAnswer(CGXXmlWriter *writer, CGXDLMSAutoAnswer *obj) {
    ...
    if ((ret = writer->WriteElementString("Mode", obj->GetMode())) == 0 &&
        ...
        for (std::vector<std::pair<CGXDateTime, CGXDateTime>>::iterator it = obj->GetListeningWindow().begin();
             it != obj->GetListeningWindow().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
        ...
        if ((ret == 0)) {
            if ((ret = writer->WriteEndElement()) == 0 &&
                ...
                (ret = writer->WriteElementString(
                     "NumberOfRingsInListeningWindow", obj->GetNumberOfRingsInListeningWindow()
                 )) == 0 &&
                (ret = writer->WriteElementString(
                     "NumberOfRingsOutListeningWindow", obj->GetNumberOfRingsOutListeningWindow()
                 )) == 0) {
            }
        }
    }
...
int SaveAutoConnect(CGXXmlWriter *writer, CGXDLMSAutoConnect *obj) {
    ...
    if ((ret = writer->WriteElementString("Mode", obj->GetMode())) == 0 &&
        ...
        for (std::vector<std::pair<CGXDateTime, CGXDateTime>>::iterator it = obj->GetCallingWindow().begin();
             it != obj->GetCallingWindow().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
                (ret = writer->WriteElementString("Start", it->first)) != 0 ||
                (ret = writer->WriteElementString("End", it->second)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
...
int SaveExtendedRegister(CGXXmlWriter *writer, CGXDLMSExtendedRegister *obj) {
    ...
    if ((ret = writer->WriteElementString("Unit", obj->GetUnit())) != 0 ||
        (ret = writer->WriteElementString("Scaler", obj->GetScaler(), 1)) != 0 ||
        (ret = obj->GetDataType(2, dt)) != 0 || (ret = obj->GetUIDataType(2, uiDt)) != 0 ||
...
int SaveGprsSetup(CGXXmlWriter *writer, CGXDLMSGPRSSetup *obj) {
    ...
    if ((ret = writer->WriteElementString("APN", obj->GetAPN())) != 0 ||
        ...
        (ret = writer->WriteElementString("PeakThroughput", obj->GetDefaultQualityOfService().GetPeakThroughput())) !=
            0 ||
        (ret = writer->WriteElementString("MeanThroughput", obj->GetDefaultQualityOfService().GetMeanThroughput())) !=
            0 ||
        ...
        (ret = writer->WriteElementString("PeakThroughput", obj->GetRequestedQualityOfService().GetPeakThroughput())) !=
            0 ||
        (ret = writer->WriteElementString("MeanThroughput", obj->GetRequestedQualityOfService().GetMeanThroughput())) !=
            0 ||
        (ret = writer->WriteEndElement()) != 0) {
    }
    return ret;
...
int SaveIp4Setup(CGXXmlWriter *writer, CGXDLMSIp4Setup *obj) {
    ...
    if ((ret = writer->WriteElementString("DataLinkLayerReference", obj->GetDataLinkLayerReference())) == 0 &&
        ...
        for (std::vector<unsigned int>::iterator it = obj->GetMulticastIPAddress().begin();
             it != obj->GetMulticastIPAddress().end(); ++it) {
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("IPOptions")) == 0) {
                for (std::vector<CGXDLMSIp4SetupIpOption>::iterator it = obj->GetIPOptions().begin();
                     it != obj->GetIPOptions().end(); ++it) {
...
int SaveGSMDiagnostic(CGXXmlWriter *writer, CGXDLMSGSMDiagnostic *obj) {
    ...
    if ((ret = writer->WriteElementString("Operator", obj->GetOperator())) == 0 &&
        ...
        (ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("AdjacentCells")) == 0) {
        for (std::vector<GXAdjacentCell *>::iterator it = obj->GetAdjacentCells().begin();
             it != obj->GetAdjacentCells().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveModemConfiguration(CGXXmlWriter *writer, CGXDLMSModemConfiguration *obj) {
    int ret;
    std::string str;
    if ((ret = writer->WriteElementString("CommunicationSpeed", obj->GetCommunicationSpeed(), DLMS_BAUD_RATE_300)) ==
            0 &&
        (ret = writer->WriteStartElement("InitialisationStrings")) == 0) {
        for (std::vector<CGXDLMSModemInitialisation>::iterator it = obj->GetInitialisationStrings().begin();
             it != obj->GetInitialisationStrings().end(); ++it) {
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 &&
                ...
                for (std::vector<std::pair<CGXDateTime, CGXDateTime>>::iterator it =
                         obj->GetCommunicationWindow().begin();
                ...
                if (ret == 0) {
                    if ((ret = writer->WriteEndElement()) != 0 ||
                        (ret = writer->WriteElementString(
                             "RandomisationStartInterval", obj->GetRandomisationStartInterval()
                         )) != 0 ||
...
int SavePppSetup(CGXXmlWriter *writer, CGXDLMSPppSetup *obj) {
    ...
    if ((ret = writer->WriteElementString("PHYReference", obj->GetPHYReference())) == 0 &&
        ...
        for (std::vector<CGXDLMSPppSetupLcpOption>::iterator it = obj->GetLCPOptions().begin();
             it != obj->GetLCPOptions().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
                (ret = writer->WriteElementString("Type", it->GetType())) != 0 ||
                (ret = writer->WriteElementString("Length", it->GetLength())) != 0 ||
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("IPCPOptions")) == 0) {
                for (std::vector<CGXDLMSPppSetupIPCPOption>::iterator it = obj->GetIPCPOptions().begin();
                     it != obj->GetIPCPOptions().end(); ++it) {
                    if ((ret = writer->WriteStartElement("Item")) != 0 ||
                        (ret = writer->WriteElementString("Type", it->GetType())) != 0 ||
                        (ret = writer->WriteElementString("Length", it->GetLength())) != 0 ||
                        (ret = writer->WriteElementObject("Data", it->GetData())) != 0 ||
...
int SaveRegisterActivation(CGXXmlWriter *writer, CGXDLMSRegisterActivation *obj) {
    ...
    if ((ret = writer->WriteStartElement("RegisterAssignment")) == 0) {
        for (std::vector<CGXDLMSObjectDefinition>::iterator it = obj->GetRegisterAssignment().begin();
             it != obj->GetRegisterAssignment().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("MaskList")) == 0) {
                std::string str;
                for (std::vector<std::pair<CGXByteBuffer, CGXByteBuffer>>::iterator it = obj->GetMaskList().begin();
                     it != obj->GetMaskList().end(); ++it) {
...
int SaveRegisterMonitor(CGXXmlWriter *writer, CGXDLMSRegisterMonitor *obj) {
    ...
    if ((ret = writer->WriteStartElement("Thresholds")) == 0) {
        for (std::vector<CGXDLMSVariant>::iterator it = obj->GetThresholds().begin(); it != obj->GetThresholds().end();
             ++it) {
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("MonitoredValue")) == 0 &&
                ...
                (ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("Actions")) == 0) {
                for (std::vector<CGXDLMSActionSet *>::iterator it = obj->GetActions().begin();
                    ...
                    if ((ret = writer->WriteStartElement("Item")) != 0 ||
                        ...
                        (ret = writer->WriteElementString("Selector", (*it)->GetActionDown().GetScriptSelector())) !=
                            0 ||
...
int SaveLimiter(CGXXmlWriter *writer, CGXDLMSLimiter *obj) {
    ...
    if ((ret = writer->WriteStartElement("MonitoredValue")) == 0 &&
        ...
        (ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("EmergencyProfileGroupIDs")) == 0) {
        for (std::vector<int>::iterator it = obj->GetEmergencyProfileGroupIDs().begin();
             it != obj->GetEmergencyProfileGroupIDs().end(); ++it) {
            if ((ret = writer->WriteElementString("Value", *it)) != 0) {
                break;
            }
        }
...
int SaveMBusClient(CGXXmlWriter *writer, CGXDLMSMBusClient *obj) {
    ...
    if ((ret = writer->WriteElementString("MBusPortReference", obj->GetMBusPortReference())) == 0 &&
        ...
        for (std::vector<std::pair<std::string, std::string>>::iterator it = obj->GetCaptureDefinition().begin();
             it != obj->GetCaptureDefinition().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveMulticastEntries(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("MulticastEntries")) == 0) {
        for (std::vector<CGXMacMulticastEntry *>::iterator it = obj->GetMulticastEntries().begin();
             it != obj->GetMulticastEntries().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveDirectTable(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("DirectTable")) == 0) {
        for (std::vector<CGXMacDirectTable *>::iterator it = obj->GetDirectTable().begin();
             it != obj->GetDirectTable().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveAvailableSwitches(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("AvailableSwitches")) == 0) {
        for (std::vector<CGXMacAvailableSwitch *>::iterator it = obj->GetAvailableSwitches().begin();
             it != obj->GetAvailableSwitches().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveCommunications(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("Communications")) == 0) {
        for (std::vector<CGXMacPhyCommunication *>::iterator it = obj->GetCommunications().begin();
             it != obj->GetCommunications().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SavePrimeNbOfdmPlcMacNetworkAdministrationData(
    CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj
) {
    int ret;
    if ((ret = SaveMulticastEntries(writer, obj)) != 0 || (ret = SaveSwitchTable(writer, obj)) != 0 ||
        (ret = SaveDirectTable(writer, obj)) != 0 || (ret = SaveAvailableSwitches(writer, obj)) != 0 ||
...
#ifndef DLMS_IGNORE_PRIME_NB_OFDM_PLC_APPLICATIONS_IDENTIFICATION
int SavePrimeNbOfdmPlcApplicationsIdentification(
    CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcApplicationsIdentification *obj
) {
    int ret;
...
int SaveParameterMonitor(CGXXmlWriter *writer, CGXDLMSParameterMonitor *obj) {
    ...
    if (ret == 0) {
        if ((ret = writer->WriteElementString("Time", obj->GetCaptureTime())) == 0 &&
            ...
            for (std::vector<CGXDLMSTarget *>::iterator it = obj->GetParameters().begin();
                 it != obj->GetParameters().end(); ++it) {
...
int SaveSFSKReportingSystemList(CGXXmlWriter *writer, CGXDLMSSFSKReportingSystemList *obj) {
    ...
    if ((ret = writer->WriteStartElement("ReportingSystems")) == 0) {
        for (std::vector<CGXByteBuffer>::iterator it = obj->GetReportingSystemList().begin();
             it != obj->GetReportingSystemList().end(); ++it) {
...
int SaveArbitrator(CGXXmlWriter *writer, CGXDLMSArbitrator *obj) {
    ...
    if ((ret = writer->WriteStartElement("Actions")) == 0) {
        for (std::vector<CGXDLMSActionItem>::iterator it = obj->GetActions().begin(); it != obj->GetActions().end();
             ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
                (ret = writer->WriteElementString("LN", it->GetLogicalName())) != 0 ||
...
int SaveSFSKMacCounters(CGXXmlWriter *writer, CGXDLMSSFSKMacCounters *obj) {
    ...
    if ((ret = writer->WriteStartElement("SynchronizationRegisters")) == 0) {
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0) {
                if ((ret = writer->WriteElementString(
                         "PhysicalLayerDesynchronization", obj->GetPhysicalLayerDesynchronization()
                     )) == 0 &&
                    (ret = writer->WriteElementString(
                         "TimeOutNotAddressedDesynchronization", obj->GetTimeOutNotAddressedDesynchronization()
                     )) == 0 &&
                    (ret = writer->WriteElementString(
                         "TimeOutFrameNotOkDesynchronization", obj->GetTimeOutFrameNotOkDesynchronization()
                     )) == 0 &&
                    (ret = writer->WriteElementString(
                         "WriteRequestDesynchronization", obj->GetWriteRequestDesynchronization()
                     )) == 0 &&
                    (ret = writer->WriteElementString(
                         "WrongInitiatorDesynchronization", obj->GetWrongInitiatorDesynchronization()
                     )) == 0 &&
                    (ret = writer->WriteStartElement("BroadcastFramesCounters")) == 0) {
                    for (std::vector<std::pair<uint16_t, uint32_t>>::iterator it =
                             obj->GetBroadcastFramesCounter().begin();
...
int SaveSFSKMacSynchronizationTimeouts(CGXXmlWriter *writer, CGXDLMSSFSKMacSynchronizationTimeouts *obj) {
    ...
    if ((ret = writer->WriteElementString("SearchInitiatorTimeout", obj->GetSearchInitiatorTimeout())) != 0 ||
        (ret = writer->WriteElementString(
             "SynchronizationConfirmationTimeout", obj->GetSynchronizationConfirmationTimeout()
         )) != 0 ||
...
int SaveSFSKPhyMacSetUp(CGXXmlWriter *writer, CGXDLMSSFSKPhyMacSetUp *obj) {
    ...
    if ((ret = writer->WriteElementString("InitiatorElectricalPhase", obj->GetInitiatorElectricalPhase())) == 0 &&
        ...
        if (ret == 0 && (ret = writer->WriteEndElement()) == 0 &&
...
int SaveNtpSetup(CGXXmlWriter *writer, CGXDLMSNtpSetup *obj) {
    ...
    if ((ret = writer->WriteElementString("Activated", obj->GetActivated())) == 0 &&
        ...
        for (std::map<uint32_t, CGXByteBuffer>::iterator it = obj->GetKeys().begin(); it != obj->GetKeys().end();
             ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveReferences(CGXXmlWriter *writer, std::vector<std::string> &list, const char *name) {
    ...
    if ((ret = writer->WriteStartElement(name)) == 0) {
        for (std::vector<std::string>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
                (ret = writer->WriteElementString("Name", *it)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveAccount(CGXXmlWriter *writer, CGXDLMSAccount *obj) {
    ...
    if ((ret = writer->WriteElementString("PaymentMode", obj->GetPaymentMode())) != 0 ||
        ...
        (ret = writer->WriteElementString("NextCreditAvailableThreshold", obj->GetNextCreditAvailableThreshold())) !=
            0 ||
...
int SaveTokenGateway(CGXXmlWriter *writer, CGXDLMSTokenGateway *obj) {
    ...
    if ((ret = writer->WriteElementString("Token", obj->GetToken().ToHexString())) == 0 &&
        ...
        for (std::vector<std::string>::iterator it = obj->GetDescriptions().begin(); it != obj->GetDescriptions().end();
             ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
                (ret = writer->WriteElementString("Name", *it)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
...
int SaveSapAssignment(CGXXmlWriter *writer, CGXDLMSSapAssignment *obj) {
    ...
    if ((ret = writer->WriteStartElement("SapAssignmentList")) == 0) {
        for (std::map<int, std::string>::iterator it = obj->GetSapAssignmentList().begin();
             it != obj->GetSapAssignmentList().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveImageTransfer(CGXXmlWriter *writer, CGXDLMSImageTransfer *obj) {
    ...
    if ((ret = writer->WriteElementString("ImageBlockSize", obj->GetImageBlockSize())) == 0 &&
        (ret = writer->WriteElementString("ImageTransferredBlocksStatus", obj->GetImageTransferredBlocksStatus())) ==
            0 &&
        (ret = writer->WriteElementString(
             "ImageFirstNotTransferredBlockNumber", obj->GetImageFirstNotTransferredBlockNumber()
         )) == 0 &&
        ...
        for (std::vector<CGXDLMSImageActivateInfo *>::iterator it = obj->GetImageActivateInfo().begin();
             it != obj->GetImageActivateInfo().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveSchedule(CGXXmlWriter *writer, CGXDLMSSchedule *obj) {
    ...
    if ((ret = writer->WriteStartElement("Entries")) == 0) {
        for (std::vector<CGXDLMSScheduleEntry *>::iterator it = obj->GetEntries().begin();
             it != obj->GetEntries().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 ||
...
int SaveScriptTable(CGXXmlWriter *writer, CGXDLMSScriptTable *obj) {
    ...
    if ((ret = writer->WriteStartElement("Scripts")) == 0) {
        for (std::vector<CGXDLMSScript *>::iterator it = obj->GetScripts().begin(); it != obj->GetScripts().end();
             ++it) {
...
int SaveSpecialDaysTable(CGXXmlWriter *writer, CGXDLMSSpecialDaysTable *obj) {
    int ret;
    if ((ret = writer->WriteStartElement("Entries")) == 0) {
        for (std::vector<CGXDLMSSpecialDay *>::iterator it = obj->GetEntries().begin(); it != obj->GetEntries().end();
             ++it) {
...
*/
        ...
        case DLMS_OBJECT_TYPE_PRIME_NB_OFDM_PLC_MAC_NETWORK_ADMINISTRATION_DATA:
            return SavePrimeNbOfdmPlcMacNetworkAdministrationData(
                this, (CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *)obj
            );
#endif  //DLMS_IGNORE_PRIME_NB_OFDM_PLC_MAC_NETWORK_ADMINISTRATION_DATA
#ifndef DLMS_IGNORE_PRIME_NB_OFDM_PLC_APPLICATIONS_IDENTIFICATION
        case DLMS_OBJECT_TYPE_PRIME_NB_OFDM_PLC_APPLICATIONS_IDENTIFICATION:
            return SavePrimeNbOfdmPlcApplicationsIdentification(
                this, (CGXDLMSPrimeNbOfdmPlcApplicationsIdentification *)obj
            );
#endif  //DLMS_IGNORE_PRIME_NB_OFDM_PLC_APPLICATIONS_IDENTIFICATION
#ifndef DLMS_IGNORE_IEC_8802_LLC_TYPE1_SETUP
