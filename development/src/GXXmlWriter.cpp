...
int CGXXmlWriter::WriteArray(CGXDLMSVariant &data) {
    ...
    if (data.vt == DLMS_DATA_TYPE_ARRAY || data.vt == DLMS_DATA_TYPE_STRUCTURE) {
        for (std::vector<CGXDLMSVariant>::iterator it = data.Arr.begin(); it != data.Arr.end(); ++it) {
            ...
            } else if (it->vt == DLMS_DATA_TYPE_ARRAY || it->vt == DLMS_DATA_TYPE_STRUCTURE) {
                AppendSpaces();
                m_Elements.push_back("Item");
                fprintf(m_f, "<%s Type=\"%d\"", "Item", it->vt);
                if ((ret = WriteArray(*it)) != 0 || (ret = WriteEndElement()) != 0) {
                    break;
                }
            } else {
                if ((ret = WriteElementObject("Item", *it)) != 0) {
...
int SaveActionSchedule(CGXXmlWriter *writer, CGXDLMSActionSchedule *obj) {
    ...
    if ((ret = writer->WriteElementString("LN", obj->GetExecutedScriptLogicalName())) == 0 &&
        ...
        (ret = writer->WriteElementString("Type", (int)obj->GetType())) == 0 && (ret = writer->WriteStartElement("ExecutionTime")) == 0) {
...
int SaveSeasonProfile(CGXXmlWriter *writer, std::vector<CGXDLMSSeasonProfile *> &list, const char *name) {
    ...
    if ((ret = writer->WriteStartElement(name)) == 0) {
        for (std::vector<CGXDLMSSeasonProfile *>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Name", (*it)->GetName().ToHexString())) != 0 ||
                //Some meters are returning time here, not date-time.
                (ret = writer->WriteElementString("Start", (*it)->GetStart())) != 0 ||
                (ret = writer->WriteElementString("WeekName", (*it)->GetWeekName().ToHexString())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveWeekProfileTable(CGXXmlWriter *writer, std::vector<CGXDLMSWeekProfile *> &list, const char *name) {
    ...
    if ((ret = writer->WriteStartElement(name)) == 0) {
        ...
        for (std::vector<CGXDLMSWeekProfile *>::iterator it = list.begin(); it != list.end(); ++it) {
            (*it)->GetName(bb);
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Name", bb.ToHexString())) != 0 ||
                ...
                (ret = writer->WriteElementString("Sunday", (*it)->GetSunday())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveDayProfileTable(CGXXmlWriter *writer, std::vector<CGXDLMSDayProfile *> &list, const char *name) {
    ...
    if ((ret = writer->WriteStartElement(name)) == 0) {
        for (std::vector<CGXDLMSDayProfile *>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) == 0 && (ret = writer->WriteElementString("DayId", (*it)->GetDayId())) == 0 &&
                ...
                for (std::vector<CGXDLMSDayProfileAction *>::iterator d = (*it)->GetDaySchedules().begin(); d != (*it)->GetDaySchedules().end(); ++d) {
                    if ((ret = writer->WriteStartElement("Action")) != 0 || (ret = writer->WriteElementString("Start", (*d)->GetStartTime())) != 0 ||
                        (ret = writer->WriteElementString("LN", (*d)->GetScriptLogicalName())) != 0 ||
                        (ret = writer->WriteElementString("Selector", (*d)->GetScriptSelector())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                        break;
                    }
                }
            }
...
int SaveAssociationLogicalName(CGXXmlWriter *writer, CGXDLMSAssociationLogicalName *obj) {
    ...
    if ((ret = writer->WriteElementString("ClientSAP", obj->GetClientSAP())) == 0 &&
        (ret = writer->WriteElementString("ServerSAP", obj->GetServerSAP())) == 0 && (ret = writer->WriteStartElement("ApplicationContextName")) == 0 &&
        ...
        (ret = writer->WriteElementString("ContextId", obj->GetApplicationContextName().GetContextId())) == 0 && (ret = writer->WriteEndElement()) == 0 &&
        ...
        (ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteElementString("Secret", obj->GetSecret().ToHexString())) == 0 &&
...
int SaveAutoAnswer(CGXXmlWriter *writer, CGXDLMSAutoAnswer *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Mode", obj->GetMode())) == 0 && (ret = writer->WriteStartElement("ListeningWindow")) == 0) {
        for (std::vector<std::pair<CGXDateTime, CGXDateTime>>::iterator it = obj->GetListeningWindow().begin(); it != obj->GetListeningWindow().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Start", it->first)) != 0 ||
                (ret = writer->WriteElementString("End", it->second)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if ((ret == 0)) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteElementString("Status", obj->GetStatus())) == 0 &&
...
int SaveAutoConnect(CGXXmlWriter *writer, CGXDLMSAutoConnect *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Mode", obj->GetMode())) == 0 && (ret = writer->WriteElementString("Repetitions", obj->GetRepetitions())) == 0 &&
        (ret = writer->WriteElementString("RepetitionDelay", obj->GetRepetitionDelay())) == 0 && (ret = writer->WriteStartElement("CallingWindow")) == 0) {
...
int SaveClock(CGXXmlWriter *writer, CGXDLMSClock *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Time", obj->GetTime())) == 0 && (ret = writer->WriteElementString("TimeZone", obj->GetTimeZone())) == 0 &&
        (ret = writer->WriteElementString("Status", obj->GetStatus())) == 0 && (ret = writer->WriteElementString("Begin", obj->GetBegin())) == 0 &&
        (ret = writer->WriteElementString("End", obj->GetEnd())) == 0 && (ret = writer->WriteElementString("Deviation", obj->GetDeviation())) == 0 &&
        (ret = writer->WriteElementString("Enabled", obj->GetEnabled())) == 0 && (ret = writer->WriteElementString("ClockBase", obj->GetClockBase())) == 0) {
    }
    return ret;
}
#endif  //DLMS_IGNORE_CLOCK
...
int SaveData(CGXXmlWriter *writer, CGXDLMSData *obj) {
    ...
    if ((ret = obj->GetDataType(2, dt)) != 0 || (ret = obj->GetUIDataType(2, uiDt)) != 0 ||
...
int SaveDemandRegister(CGXXmlWriter *writer, CGXDLMSDemandRegister *obj) {
    ...
    if ((ret = writer->WriteElementObject("CurrentAverageValue", obj->GetCurrentAverageValue())) != 0 ||
        ...
        (ret = writer->WriteElementString("Scaler", obj->GetScaler(), 1)) != 0 || (ret = writer->WriteElementString("Unit", obj->GetUnit())) != 0 ||
        (ret = writer->WriteElementObject("Status", obj->GetStatus())) != 0 || (ret = writer->WriteElementString("CaptureTime", obj->GetCaptureTime())) != 0 ||
...
int SaveExtendedRegister(CGXXmlWriter *writer, CGXDLMSExtendedRegister *obj) {
    ...
    if ((ret = writer->WriteElementString("Unit", obj->GetUnit())) != 0 || (ret = writer->WriteElementString("Scaler", obj->GetScaler(), 1)) != 0 ||
        (ret = obj->GetDataType(2, dt)) != 0 || (ret = obj->GetUIDataType(2, uiDt)) != 0 || (ret = writer->WriteElementObject("Value", value, dt, uiDt)) != 0 ||
        (ret = writer->WriteElementObject("Status", obj->GetStatus())) != 0 || (ret = writer->WriteElementString("CaptureTime", obj->GetCaptureTime())) != 0) {
    }
    return ret;
}
...
int SaveGprsSetup(CGXXmlWriter *writer, CGXDLMSGPRSSetup *obj) {
    int ret;
    if ((ret = writer->WriteElementString("APN", obj->GetAPN())) != 0 || (ret = writer->WriteElementString("PINCode", obj->GetPINCode())) != 0 ||
        ...
        (ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteStartElement("RequestedQualityOfService")) != 0 ||
...
int SaveIecTwistedPairSetup(CGXXmlWriter *writer, CGXDLMSIecTwistedPairSetup *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Mode", (int)obj->GetMode())) == 0 && (ret = writer->WriteElementString("Speed", (int)obj->GetSpeed())) == 0) {
        CGXByteBuffer bb;
        for (std::vector<unsigned char>::iterator it = obj->GetPrimaryAddresses().begin(); it != obj->GetPrimaryAddresses().end(); ++it) {
            bb.SetUInt8(*it);
        }
        if ((ret = writer->WriteElementString("PrimaryAddresses", bb.ToHexString(true))) == 0) {
            bb.Clear();
            for (std::vector<char>::iterator it = obj->GetTabis().begin(); it != obj->GetTabis().end(); ++it) {
                bb.SetUInt8(*it);
            }
...
int SaveIp4Setup(CGXXmlWriter *writer, CGXDLMSIp4Setup *obj) {
    ...
    if ((ret = writer->WriteElementString("DataLinkLayerReference", obj->GetDataLinkLayerReference())) == 0 &&
        (ret = writer->WriteElementString("IPAddress", str)) == 0 && (ret = writer->WriteStartElement("MulticastIPAddress")) == 0) {
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("IPOptions")) == 0) {
                for (std::vector<CGXDLMSIp4SetupIpOption>::iterator it = obj->GetIPOptions().begin(); it != obj->GetIPOptions().end(); ++it) {
                    if ((ret = writer->WriteStartElement("IPOptions")) != 0 || (ret = writer->WriteElementString("Type", it->GetType())) != 0 ||
                        (ret = writer->WriteElementString("Length", it->GetLength())) != 0 ||
                        (ret = writer->WriteElementString("Data", it->GetData().ToHexString())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                        break;
                    }
                }
                if (ret == 0) {
                    obj->GetSubnetMask(str);
                    if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteElementString("SubnetMask", str)) == 0) {
...
int SaveGSMDiagnostic(CGXXmlWriter *writer, CGXDLMSGSMDiagnostic *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Operator", obj->GetOperator())) == 0 && (ret = writer->WriteElementString("Status", obj->GetStatus())) == 0 &&
        (ret = writer->WriteElementString("CircuitSwitchStatus", obj->GetCircuitSwitchStatus())) == 0 &&
        (ret = writer->WriteElementString("PacketSwitchStatus", obj->GetPacketSwitchStatus())) == 0 && (ret = writer->WriteStartElement("CellInfo")) == 0 &&
        ...
        (ret = writer->WriteElementString("Ber", obj->GetCellInfo().GetBer())) == 0 && (ret = writer->WriteEndElement()) == 0 &&
        ...
        for (std::vector<GXAdjacentCell *>::iterator it = obj->GetAdjacentCells().begin(); it != obj->GetAdjacentCells().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("CellId", (*it)->GetCellId())) != 0 ||
                (ret = writer->WriteElementString("SignalQuality", (*it)->GetSignalQuality())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveModemConfiguration(CGXXmlWriter *writer, CGXDLMSModemConfiguration *obj) {
    ...
    if ((ret = writer->WriteElementString("CommunicationSpeed", obj->GetCommunicationSpeed(), DLMS_BAUD_RATE_300)) == 0 &&
        ...
        for (std::vector<CGXDLMSModemInitialisation>::iterator it = obj->GetInitialisationStrings().begin(); it != obj->GetInitialisationStrings().end();
             ++it) {
            if ((ret = writer->WriteStartElement("Initialisation")) != 0 || (ret = writer->WriteElementString("Request", it->GetRequest())) != 0 ||
                (ret = writer->WriteElementString("Response", it->GetResponse())) != 0 || (ret = writer->WriteElementString("Delay", it->GetDelay())) != 0 ||
                (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
...
int SavePushSetup(CGXXmlWriter *writer, CGXDLMSPushSetup *obj) {
    ...
    if ((ret = writer->WriteStartElement("ObjectList")) == 0) {
        for (std::vector<std::pair<CGXDLMSObject *, CGXDLMSCaptureObject>>::iterator it = obj->GetPushObjectList().begin();
             it != obj->GetPushObjectList().end(); ++it) {
            it->first->GetLogicalName(str);
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("ObjectType", it->first->GetObjectType())) != 0 ||
                (ret = writer->WriteElementString("LN", str)) != 0 || (ret = writer->WriteElementString("AI", it->second.GetAttributeIndex())) != 0 ||
                (ret = writer->WriteElementString("DI", it->second.GetDataIndex())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteElementString("Service", (int)obj->GetService())) == 0 &&
                (ret = writer->WriteElementString("Destination", obj->GetDestination())) == 0 &&
                (ret = writer->WriteElementString("Message", obj->GetMessageType())) == 0 && (ret = writer->WriteStartElement("CommunicationWindow")) == 0) {
                for (std::vector<std::pair<CGXDateTime, CGXDateTime>>::iterator it = obj->GetCommunicationWindow().begin();
                     it != obj->GetCommunicationWindow().end(); ++it) {
                    if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Start", it->first)) != 0 ||
                        (ret = writer->WriteElementString("End", it->second)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                        break;
                    }
...
int SavePppSetup(CGXXmlWriter *writer, CGXDLMSPppSetup *obj) {
    int ret;
    if ((ret = writer->WriteElementString("PHYReference", obj->GetPHYReference())) == 0 && (ret = writer->WriteStartElement("LCPOptions")) == 0) {
        for (std::vector<CGXDLMSPppSetupLcpOption>::iterator it = obj->GetLCPOptions().begin(); it != obj->GetLCPOptions().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Type", it->GetType())) != 0 ||
                (ret = writer->WriteElementString("Length", it->GetLength())) != 0 || (ret = writer->WriteElementObject("Data", it->GetData())) != 0 ||
                (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("IPCPOptions")) == 0) {
    ...
    if (ret == 0) {
        ...
        if ((ret = writer->WriteStartElement("CaptureObjects")) == 0) {
            for (std::vector<std::pair<CGXDLMSObject *, CGXDLMSCaptureObject *>>::iterator it = obj->GetCaptureObjects().begin();
                ...
                if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("ObjectType", it->first->GetObjectType())) != 0 ||
                    ...
                    (ret = writer->WriteElementString("Data", it->second->GetDataIndex())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                    break;
                }
            }
            if (ret == 0) {
        ...
        if (ret == 0) {
            ...
            if ((ret = writer->WriteElementString("CapturePeriod", obj->GetCapturePeriod())) != 0 ||
                (ret = writer->WriteElementString("SortMethod", obj->GetSortMethod())) != 0 || (ret = writer->WriteStartElement("SortObject")) != 0 ||
                (ret = writer->WriteElementString("ObjectType", ot)) != 0 || (ret = writer->WriteElementString("LN", ln)) != 0 ||
                (ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteElementString("EntriesInUse", obj->GetEntriesInUse())) != 0 ||
...
int SaveRegister(CGXXmlWriter *writer, CGXDLMSRegister *obj) {
    DLMS_DATA_TYPE dt, uiDt;
    int ret;
    if ((ret = obj->GetDataType(2, dt)) != 0 || (ret = obj->GetUIDataType(2, uiDt)) != 0 || (ret = writer->WriteElementString("Unit", obj->GetUnit())) != 0 ||
        (ret = writer->WriteElementString("Scaler", obj->GetScaler(), 1)) != 0 || (ret = writer->WriteElementObject("Value", obj->GetValue(), dt, uiDt)) != 0) {
...
int SaveRegisterActivation(CGXXmlWriter *writer, CGXDLMSRegisterActivation *obj) {
    ...
    if ((ret = writer->WriteStartElement("RegisterAssignment")) == 0) {
        for (std::vector<CGXDLMSObjectDefinition>::iterator it = obj->GetRegisterAssignment().begin(); it != obj->GetRegisterAssignment().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("ObjectType", it->GetObjectType())) != 0 ||
                (ret = writer->WriteElementString("LN", it->GetLogicalName())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("MaskList")) == 0) {
                std::string str;
                for (std::vector<std::pair<CGXByteBuffer, CGXByteBuffer>>::iterator it = obj->GetMaskList().begin(); it != obj->GetMaskList().end(); ++it) {
                    ...
                    if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Mask", it->first.ToHexString())) != 0 ||
                        (ret = writer->WriteElementString("Index", str)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                        break;
                    }
                }
                if (ret == 0) {
...
int SaveRegisterMonitor(CGXXmlWriter *writer, CGXDLMSRegisterMonitor *obj) {
    ...
    if ((ret = writer->WriteStartElement("Thresholds")) == 0) {
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteStartElement("MonitoredValue")) == 0 &&
                ...
                (ret = writer->WriteElementString("Index", obj->GetMonitoredValue().GetAttributeIndex())) == 0 && (ret = writer->WriteEndElement()) == 0 &&
                (ret = writer->WriteStartElement("Actions")) == 0) {
                for (std::vector<CGXDLMSActionSet *>::iterator it = obj->GetActions().begin(); it != obj->GetActions().end(); ++it) {
                    if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteStartElement("Up")) != 0 ||
                        ...
                        (ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteStartElement("Down")) != 0 ||
                        ...
                        (ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteEndElement()) != 0) {
                        break;
                    }
                }
                if (ret == 0) {
...
int SaveLimiter(CGXXmlWriter *writer, CGXDLMSLimiter *obj) {
    ...
    if ((ret = writer->WriteStartElement("MonitoredValue")) == 0 && (ret = writer->WriteElementString("ObjectType", ot)) == 0 &&
        (ret = writer->WriteElementString("LN", ln)) == 0 && (ret = writer->WriteEndElement()) == 0 &&
        ...
        (ret = writer->WriteStartElement("EmergencyProfile")) == 0 && (ret = writer->WriteElementString("ID", obj->GetEmergencyProfile().GetID())) == 0 &&
        (ret = writer->WriteElementString("Time", obj->GetEmergencyProfile().GetActivationTime())) == 0 &&
        (ret = writer->WriteElementString("Duration", obj->GetEmergencyProfile().GetDuration())) == 0 && (ret = writer->WriteEndElement()) == 0 &&
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteElementString("Active", obj->GetEmergencyProfileActive())) != 0 ||
                ...
                (ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteStartElement("ActionUnderThreshold")) != 0 ||
...
int SaveMBusClient(CGXXmlWriter *writer, CGXDLMSMBusClient *obj) {
    ...
    if ((ret = writer->WriteElementString("MBusPortReference", obj->GetMBusPortReference())) == 0 &&
        ...
        for (std::vector<std::pair<std::string, std::string>>::iterator it = obj->GetCaptureDefinition().begin(); it != obj->GetCaptureDefinition().end();
             ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Data", it->first)) != 0 ||
                (ret = writer->WriteElementString("Value", it->second)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteElementString("CapturePeriod", obj->GetCapturePeriod())) == 0 &&
                ...
                (ret = writer->WriteElementString("Status", obj->GetStatus())) == 0 && (ret = writer->WriteElementString("Alarm", obj->GetAlarm())) == 0) {
                if (obj->GetVersion() > 0) {
...
int SavePrimeNbOfdmPlcMacFunctionalParameters(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacFunctionalParameters *obj) {
    int ret;
    if ((ret = writer->WriteElementString("LnId", obj->GetLnId())) != 0 || (ret = writer->WriteElementString("LsId", obj->GetLsId())) != 0 ||
        (ret = writer->WriteElementString("SId", obj->GetSId())) != 0 || (ret = writer->WriteElementString("SNa", obj->GetSna().ToHexString())) != 0 ||
        (ret = writer->WriteElementString("State", obj->GetState())) != 0 || (ret = writer->WriteElementString("ScpLength", obj->GetScpLength())) != 0 ||
...
int SaveMulticastEntries(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("MulticastEntries")) == 0) {
        for (std::vector<CGXMacMulticastEntry *>::iterator it = obj->GetMulticastEntries().begin(); it != obj->GetMulticastEntries().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Id", (*it)->GetId())) != 0 ||
                (ret = writer->WriteElementString("Members", (*it)->GetMembers())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveDirectTable(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("DirectTable")) == 0) {
        for (std::vector<CGXMacDirectTable *>::iterator it = obj->GetDirectTable().begin(); it != obj->GetDirectTable().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("SourceSId", (*it)->GetSourceSId())) != 0 ||
                ...
                (ret = writer->WriteElementString("Did", (*it)->GetDid().ToHexString())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveAvailableSwitches(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("AvailableSwitches")) == 0) {
        for (std::vector<CGXMacAvailableSwitch *>::iterator it = obj->GetAvailableSwitches().begin(); it != obj->GetAvailableSwitches().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Sna", (*it)->GetSna().ToHexString())) != 0 ||
                (ret = writer->WriteElementString("LsId", (*it)->GetLsId())) != 0 || (ret = writer->WriteElementString("Level", (*it)->GetLevel())) != 0 ||
                (ret = writer->WriteElementString("RxLevel", (*it)->GetRxLevel())) != 0 ||
                (ret = writer->WriteElementString("RxSnr", (*it)->GetRxSnr())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveCommunications(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    ...
    if ((ret = writer->WriteStartElement("Communications")) == 0) {
        for (std::vector<CGXMacPhyCommunication *>::iterator it = obj->GetCommunications().begin(); it != obj->GetCommunications().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Eui", (*it)->GetEui().ToHexString())) != 0 ||
                ...
                (ret = writer->WriteElementString("RxLvl", (*it)->GetRxLvl())) != 0 || (ret = writer->WriteElementString("Snr", (*it)->GetSnr())) != 0 ||
                ...
                (ret = writer->WriteElementString("RxCodingModified", (*it)->GetRxCodingModified())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SavePrimeNbOfdmPlcMacNetworkAdministrationData(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcMacNetworkAdministrationData *obj) {
    int ret;
    if ((ret = SaveMulticastEntries(writer, obj)) != 0 || (ret = SaveSwitchTable(writer, obj)) != 0 || (ret = SaveDirectTable(writer, obj)) != 0 ||
        (ret = SaveAvailableSwitches(writer, obj)) != 0 || (ret = SaveCommunications(writer, obj)) != 0) {
    }
    return ret;
}
...
int SavePrimeNbOfdmPlcApplicationsIdentification(CGXXmlWriter *writer, CGXDLMSPrimeNbOfdmPlcApplicationsIdentification *obj) {
    ...
    if ((ret = writer->WriteElementString("FirmwareVersion", obj->GetFirmwareVersion())) != 0 ||
        (ret = writer->WriteElementString("VendorId", obj->GetVendorId())) != 0 || (ret = writer->WriteElementString("ProductId", obj->GetProductId())) != 0) {
    }
    return ret;
}
#endif  //DLMS_IGNORE_PRIME_NB_OFDM_PLC_APPLICATIONS_IDENTIFICATION
...
int SaveParameterMonitor(CGXXmlWriter *writer, CGXDLMSParameterMonitor *obj) {
    ...
    if (obj->GetChangedParameter().GetTarget() != NULL) {
        ...
        if ((ret = writer->WriteStartElement("ChangedParameter")) != 0 ||
            ...
            (ret = writer->WriteElementObject("Value", obj->GetChangedParameter().GetValue())) != 0 || (ret = writer->WriteEndElement()) != 0) {
        }
    }
    if (ret == 0) {
        if ((ret = writer->WriteElementString("Time", obj->GetCaptureTime())) == 0 && (ret = writer->WriteStartElement("Parameters")) == 0) {
            for (std::vector<CGXDLMSTarget *>::iterator it = obj->GetParameters().begin(); it != obj->GetParameters().end(); ++it) {
                ...
                if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Type", (*it)->GetTarget()->GetObjectType())) != 0 ||
                    (ret = writer->WriteElementString("LN", ln)) != 0 || (ret = writer->WriteElementString("Index", (*it)->GetAttributeIndex())) != 0 ||
                    (ret = writer->WriteEndElement()) != 0) {
                    break;
                }
            }
...
int SaveArbitrator(CGXXmlWriter *writer, CGXDLMSArbitrator *obj) {
    ...
    if ((ret = writer->WriteStartElement("Actions")) == 0) {
        for (std::vector<CGXDLMSActionItem>::iterator it = obj->GetActions().begin(); it != obj->GetActions().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("LN", it->GetLogicalName())) != 0 ||
                (ret = writer->WriteElementString("ScriptSelector", it->GetScriptSelector())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            for (std::vector<std::string>::iterator it = obj->GetPermissionsTable().begin(); it != obj->GetPermissionsTable().end(); ++it) {
                if ((ret = writer->WriteElementString("Item", *it)) != 0) {
                    break;
                }
            }
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) == 0 &&  //PermissionTable
                ...
                for (std::vector<std::vector<uint16_t>>::iterator it = obj->GetWeightingsTable().begin(); it != obj->GetWeightingsTable().end(); ++it) {
            ...
            if (ret == 0) {
                if ((ret = writer->WriteEndElement()) == 0 &&  //WeightingTable
                    ...
                    for (std::vector<std::string>::iterator it = obj->GetMostRecentRequestsTable().begin(); it != obj->GetMostRecentRequestsTable().end();
                         ++it) {
                        if ((ret = writer->WriteElementString("Item", *it)) != 0) {
                            break;
                        }
...
int SaveSFSKMacCounters(CGXXmlWriter *writer, CGXDLMSSFSKMacCounters *obj) {
    ...
    if ((ret = writer->WriteStartElement("SynchronizationRegisters")) == 0) {
        for (std::vector<std::pair<uint16_t, uint32_t>>::iterator it = obj->GetSynchronizationRegister().begin(); it != obj->GetSynchronizationRegister().end();
             ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Key", it->first)) != 0 ||
                (ret = writer->WriteElementString("Value", it->second)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
...
int SaveSFSKPhyMacSetUp(CGXXmlWriter *writer, CGXDLMSSFSKPhyMacSetUp *obj) {
    ...
    if ((ret = writer->WriteElementString("InitiatorElectricalPhase", obj->GetInitiatorElectricalPhase())) == 0 &&
        ...
        (ret = writer->WriteElementString("MacAddress", obj->GetMacAddress())) == 0 && (ret = writer->WriteStartElement("MacGroupAddresses")) == 0) {
        for (std::vector<uint16_t>::iterator it = obj->GetMacGroupAddresses().begin(); it != obj->GetMacGroupAddresses().end(); ++it) {
            if ((ret = writer->WriteElementString("Value", *it)) != 0) {
                break;
            }
        }
        if (ret == 0 && (ret = writer->WriteEndElement()) == 0 && (ret = writer->WriteElementString("Repeater", obj->GetRepeater())) == 0 &&
...
int SaveNtpSetup(CGXXmlWriter *writer, CGXDLMSNtpSetup *obj) {
    ...
    if ((ret = writer->WriteElementString("Activated", obj->GetActivated())) == 0 &&
        (ret = writer->WriteElementString("ServerAddress", obj->GetServerAddress())) == 0 && (ret = writer->WriteElementString("Port", obj->GetPort())) == 0 &&
        (ret = writer->WriteElementString("Authentication", obj->GetAuthentication())) == 0 && (ret = writer->WriteStartElement("Keys")) == 0) {
        for (std::map<uint32_t, CGXByteBuffer>::iterator it = obj->GetKeys().begin(); it != obj->GetKeys().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("ID", std::to_string(it->first))) != 0 ||
                (ret = writer->WriteElementString("Key", it->second.ToHexString())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveCommunicationPortProtection(CGXXmlWriter *writer, CGXDLMSCommunicationPortProtection *obj) {
    ...
    if ((ret = writer->WriteElementString("ProtectionMode", obj->GetProtectionMode())) == 0 &&
        ...
        (ret = writer->WriteElementString("MaxLockoutTime", obj->GetMaxLockoutTime())) == 0 && (ret = writer->WriteElementString("Port", ln)) == 0 &&
...
int SaveReferences(CGXXmlWriter *writer, std::vector<std::string> &list, const char *name) {
    ...
    if ((ret = writer->WriteStartElement(name)) == 0) {
        for (std::vector<std::string>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Name", *it)) != 0 ||
                (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
...
int SaveCreditChargeConfigurations(CGXXmlWriter *writer, std::vector<CGXCreditChargeConfiguration> &list) {
    ...
    if ((ret = writer->WriteStartElement("CreditChargeConfigurations")) == 0) {
        for (std::vector<CGXCreditChargeConfiguration>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Credit", it->GetCreditReference())) != 0 ||
                (ret = writer->WriteElementString("Charge", it->GetChargeReference())) != 0 ||
                (ret = writer->WriteElementString("Configuration", it->GetCollectionConfiguration())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveTokenGatewayConfigurations(CGXXmlWriter *writer, std::vector<CGXTokenGatewayConfiguration> &list) {
    ...
    if ((ret = writer->WriteStartElement("TokenGatewayConfigurations")) == 0) {
        for (std::vector<CGXTokenGatewayConfiguration>::iterator it = list.begin(); it != list.end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Credit", it->GetCreditReference())) != 0 ||
                (ret = writer->WriteElementString("Token", it->GetTokenProportion())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveCredit(CGXXmlWriter *writer, CGXDLMSCredit *obj) {
    ...
    if ((ret = writer->WriteElementString("CurrentCreditAmount", obj->GetCurrentCreditAmount())) != 0 ||
        (ret = writer->WriteElementString("Type", obj->GetType())) != 0 || (ret = writer->WriteElementString("Priority", obj->GetPriority())) != 0 ||
...
int SaveTokenGateway(CGXXmlWriter *writer, CGXDLMSTokenGateway *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Token", obj->GetToken().ToHexString())) == 0 && (ret = writer->WriteElementString("Time", obj->GetTime())) == 0 &&
        ...
        if (ret == 0) {
            if ((ret = writer->WriteEndElement()) != 0 || (ret = writer->WriteElementString("DeliveryMethod", obj->GetDeliveryMethod())) != 0 ||
...
int SaveSapAssignment(CGXXmlWriter *writer, CGXDLMSSapAssignment *obj) {
    ...
    if ((ret = writer->WriteStartElement("SapAssignmentList")) == 0) {
        for (std::map<int, std::string>::iterator it = obj->GetSapAssignmentList().begin(); it != obj->GetSapAssignmentList().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("SAP", it->first)) != 0 ||
                (ret = writer->WriteElementString("LDN", it->second)) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveImageTransfer(CGXXmlWriter *writer, CGXDLMSImageTransfer *obj) {
    ...
    if ((ret = writer->WriteElementString("ImageBlockSize", obj->GetImageBlockSize())) == 0 &&
        ...
        for (std::vector<CGXDLMSImageActivateInfo *>::iterator it = obj->GetImageActivateInfo().begin(); it != obj->GetImageActivateInfo().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Size", (*it)->GetSize())) != 0 ||
                (ret = writer->WriteElementString("Identification", (*it)->GetIdentification().ToHexString())) != 0 ||
                (ret = writer->WriteElementString("Signature", (*it)->GetSignature().ToHexString())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveSchedule(CGXXmlWriter *writer, CGXDLMSSchedule *obj) {
    ...
    if ((ret = writer->WriteStartElement("Entries")) == 0) {
        for (std::vector<CGXDLMSScheduleEntry *>::iterator it = obj->GetEntries().begin(); it != obj->GetEntries().end(); ++it) {
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Index", (*it)->GetIndex())) != 0 ||
                ...
                (ret = writer->WriteElementString("EndDate", (*it)->GetEndDate())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveScriptTable(CGXXmlWriter *writer, CGXDLMSScriptTable *obj) {
    ...
    if ((ret = writer->WriteStartElement("Scripts")) == 0) {
        for (std::vector<CGXDLMSScript *>::iterator it = obj->GetScripts().begin(); it != obj->GetScripts().end(); ++it) {
            if ((ret = writer->WriteStartElement("Script")) == 0 && (ret = writer->WriteElementString("ID", (*it)->GetID())) == 0 &&
                (ret = writer->WriteStartElement("Actions")) == 0) {
                for (std::vector<CGXDLMSScriptAction *>::iterator a = (*it)->GetActions().begin(); a != (*it)->GetActions().end(); ++a) {
                    if ((ret = writer->WriteStartElement("Action")) != 0 || (ret = writer->WriteElementString("Type", (*a)->GetType())) != 0 ||
                        ...
                        (ret = writer->WriteElementString("Parameter", (*a)->GetParameter().ToString())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                        break;
                    }
                }
                if (ret == 0) {
...
int SaveSpecialDaysTable(CGXXmlWriter *writer, CGXDLMSSpecialDaysTable *obj) {
    ...
    if ((ret = writer->WriteStartElement("Entries")) == 0) {
        for (std::vector<CGXDLMSSpecialDay *>::iterator it = obj->GetEntries().begin(); it != obj->GetEntries().end(); ++it) {
            if ((ret = writer->WriteStartElement("Entry")) != 0 || (ret = writer->WriteElementString("Index", (*it)->GetIndex())) != 0 ||
                (ret = writer->WriteElementString("Date", (*it)->GetDate())) != 0 || (ret = writer->WriteElementString("DayId", (*it)->GetDayId())) != 0 ||
                (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
...
int SaveSecuritySetup(CGXXmlWriter *writer, CGXDLMSSecuritySetup *obj) {
    ...
    if ((ret = writer->WriteElementString("SecurityPolicy", obj->GetSecurityPolicy())) == 0 &&
        ...
        for (std::vector<CGXDLMSCertificateInfo *>::iterator it = obj->GetCertificates().begin(); it != obj->GetCertificates().end(); ++it) {
            ...
            if ((ret = writer->WriteStartElement("Item")) != 0 || (ret = writer->WriteElementString("Entity", (*it)->GetEntity())) != 0 ||
                ...
                (ret = writer->WriteElementString("SubjectAltName", (*it)->GetSubjectAltName())) != 0 || (ret = writer->WriteEndElement()) != 0) {
                break;
            }
        }
        if (ret == 0) {
...
int SaveTcpUdpSetup(CGXXmlWriter *writer, CGXDLMSTcpUdpSetup *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Port", obj->GetPort())) != 0 || (ret = writer->WriteElementString("IPReference", obj->GetIPReference())) != 0 ||
...
int SaveUtilityTables(CGXXmlWriter *writer, CGXDLMSUtilityTables *obj) {
    int ret;
    if ((ret = writer->WriteElementString("Id", obj->GetTableId())) != 0 || (ret = writer->WriteElementString("Buffer", obj->GetBuffer().ToHexString())) != 0) {
    }
    return ret;
}
#endif  //DLMS_IGNORE_UTILITY_TABLES
