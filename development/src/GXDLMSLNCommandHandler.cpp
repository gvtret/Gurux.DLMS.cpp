...
void AppendAttributeDescriptor(
    ...
    std::string tmp;
    xml->IntegerToHex((int32_t)ci, 4, tmp);
    ...
    xml->IntegerToHex((int32_t)attributeIndex, 2, tmp);
...
int CGXDLMSLNCommandHandler::GetRequestNormal(
    ...
    if (settings.GetCount() != settings.GetIndex() || bb.GetSize() != bb.GetPosition()) {
...
int CGXDLMSLNCommandHandler::HandleSetRequestNormal(
    ...
    if (type == 2) {
        ...
        uint32_t realSize = data.GetSize() - data.GetPosition();
        if (size != realSize) {
...
int CGXDLMSLNCommandHandler::HanleSetRequestWithList(
    ...
    for (unsigned short pos = 0; pos != cnt; ++pos) {
        ...
        if (xml != NULL) {
            ...
            xml->AppendStartTag(DLMS_TRANSLATOR_TAGS_ATTRIBUTE_DESCRIPTOR);
            xml->AppendComment(CGXDLMSConverter::ToString(ci));
            xml->IntegerToHex((int32_t)ci, 4, str);
            ...
            xml->IntegerToHex((int32_t)attributeIndex, 2, str);
...
int CGXDLMSLNCommandHandler::HandleSetRequest(
    ...
    if (xml != NULL) {
        ...
        // InvokeIdAndPriority
        std::string str;
        xml->IntegerToHex((int32_t)invoke, 2, str);
...
int CGXDLMSLNCommandHandler::MethodRequestNormal(
    ...
    if (xml != NULL) {
        ...
        xml->IntegerToHex((uint32_t)invokeId, 2, str);
        xml->AppendLine(DLMS_TRANSLATOR_TAGS_INVOKE_ID, "", str);
...
int CGXDLMSLNCommandHandler::HandleEventNotification(CGXDLMSSettings &settings, CGXReplyData &reply) {
    int ret;
    unsigned char ch;
    reply.SetTime(0);
