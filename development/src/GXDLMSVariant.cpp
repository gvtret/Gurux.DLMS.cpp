...
CGXDLMSVariant::CGXDLMSVariant(CGXDLMSVariant *value) {
    ...
    if (vt == DLMS_DATA_TYPE_NONE) {
        return;
    } else if (vt == DLMS_DATA_TYPE_STRING || vt == DLMS_DATA_TYPE_BIT_STRING) {
        this->strVal.append(value->strVal);
    } else if (vt == DLMS_DATA_TYPE_DATETIME || vt == DLMS_DATA_TYPE_DATE || vt == DLMS_DATA_TYPE_TIME) {
...
CGXDLMSVariant::CGXDLMSVariant(std::wstring value) {
