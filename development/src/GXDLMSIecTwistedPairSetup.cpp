...
//Constructor.
CGXDLMSIecTwistedPairSetup::CGXDLMSIecTwistedPairSetup(): CGXDLMSIecTwistedPairSetup("0.0.23.0.0.255", 0) {
}
...
//LN Constructor.
CGXDLMSIecTwistedPairSetup::CGXDLMSIecTwistedPairSetup(std::string ln): CGXDLMSIecTwistedPairSetup(ln, 0) {
}
...
int CGXDLMSIecTwistedPairSetup::GetValue(CGXDLMSSettings &settings, CGXDLMSValueEventArg &e) {
	...
	switch (e.GetIndex()) {
		...
		case 5: {
			...
			for (std::vector<char>::iterator it = m_Tabis.begin(); it != m_Tabis.end(); ++it) {
				if ((ret = data.SetUInt8(DLMS_DATA_TYPE_INT8)) != 0 || (ret = data.SetUInt8(*it)) != 0) {
					break;
				}
			}
			e.SetValue(data);
