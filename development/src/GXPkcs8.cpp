...
int CGXPkcs8::GetEncoded(CGXByteBuffer &encoded) {
    ...
    d1->GetValues()->push_back(new CGXAsn1ObjectIdentifier(CGXDLMSConverter::ToString(m_Algorithm)));
...
int CGXPkcs8::FromPem(std::string data, CGXPkcs8 &cert) {
...
int CGXPkcs8::FromHexString(std::string &data, CGXPkcs8 &cert) {
    CGXByteBuffer bb;
    GXHelpers::HexToBytes(data, bb);
    return cert.Init(bb);
}

int CGXPkcs8::FromByteArray(CGXByteBuffer &data, CGXPkcs8 &cert) {
    return cert.Init(data);
}

int CGXPkcs8::FromByteArray(const unsigned char *data, uint16_t length, CGXPkcs8 &cert) {
    CGXByteBuffer bb;
    bb.Set(data, length);
    return cert.Init(bb);
}

int CGXPkcs8::FromDer(std::string &der, CGXPkcs8 &cert) {
