...
//Constructor.
CGXDLMSRegisterActivation::CGXDLMSRegisterActivation(): CGXDLMSRegisterActivation("", 0) {
}
...
//LN Constructor.
CGXDLMSRegisterActivation::CGXDLMSRegisterActivation(std::string ln): CGXDLMSRegisterActivation(ln, 0) {
}
...
void CGXDLMSRegisterActivation::GetValues(std::vector<std::string> &values) {
    ...
    sb << '[';
    bool empty = true;
    for (std::vector<CGXDLMSObjectDefinition>::iterator it = m_RegisterAssignment.begin();
         it != m_RegisterAssignment.end(); ++it) {
        if (!empty) {
            sb << ", ";
        }
        empty = false;
...
int CGXDLMSRegisterActivation::GetValue(CGXDLMSSettings &settings, CGXDLMSValueEventArg &e) {
    ...
    if (e.GetIndex() == 3) {
        ...
        for (std::vector<std::pair<CGXByteBuffer, CGXByteBuffer>>::iterator it = m_MaskList.begin();
             it != m_MaskList.end(); ++it) {
            data.SetUInt8(DLMS_DATA_TYPE_STRUCTURE);
            data.SetUInt8(2);
            tmp = it->first;
