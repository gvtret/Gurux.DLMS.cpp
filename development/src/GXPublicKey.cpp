...
int CGXPublicKey::FromRawBytes(CGXByteBuffer &key, CGXPublicKey &value) {
	if (key.GetSize() == 65) {
...
int CGXPublicKey::FromDer(std::string der, CGXPublicKey &key) {
...
int CGXPublicKey::FromPem(std::string pem, CGXPublicKey &key) {
...
int CGXPublicKey::Load(std::string &path, CGXPublicKey &value) {
	std::string text;
...
int CGXPublicKey::GetEncoded(CGXByteBuffer &value) {
	//Subject Key Info.
	CGXAsn1Sequence d;
