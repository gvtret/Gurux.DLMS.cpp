...
int CGXEcdsa::GetRandomNumber(CGXBigInteger &N, CGXByteBuffer &value) {
	value.SetSize(0);
...
int CGXEcdsa::Sign(CGXByteBuffer &data, CGXByteBuffer &signature) {
	signature.Clear();
	int ret;
	...
	if ((ret = GetRandomNumber(m_Curve.m_N, tmp)) == 0) {
		...
		s.Mod(m_Curve.m_N);
		tmp.Clear();
		if (ret == 0 && (ret = r.ToArray(signature, false)) == 0 && (ret = s.ToArray(tmp, false)) == 0) {
...
int CGXEcdsa::GenerateSecret(CGXPublicKey &publicKey, CGXByteBuffer &secret) {
	int ret;
	if (m_PrivateKey.GetRawValue().GetSize() == 0) {
		printf("Invalid private key.");
...
int CGXEcdsa::GenerateKeyPair(ECC scheme, std::pair<CGXPublicKey, CGXPrivateKey> &kp) {
	CGXPrivateKey pk;
	CGXPublicKey pub;
	CGXByteBuffer rn;
	CGXCurve curve;
...
int CGXEcdsa::Verify(CGXByteBuffer &signature, CGXByteBuffer &data, bool &value) {
	value = false;
	int ret;
	CGXByteBuffer bb;
