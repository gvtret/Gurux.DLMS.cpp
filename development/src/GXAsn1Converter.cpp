...
int CGXAsn1Converter::GetValue(CGXByteBuffer &bb, std::vector<CGXAsn1Base *> *objects, bool getNext) {
    int ret;
    unsigned char type;
    uint32_t len;
    CGXByteBuffer tmp2;
...
CGXDateTime CGXAsn1Converter::GetUtcTime(std::string &dateString) {
...
CGXDateTime CGXAsn1Converter::GetGeneralizedTime(std::string &dateString) {
    int year, month, day, hour, minute, second = 0;
...
int CGXAsn1Converter::GetBytes(CGXByteBuffer &bb, CGXAsn1Base *target, int &count) {
    CGXByteBuffer tmp;
    std::string str;
    int ret = 0;
...
DLMS_PKCS_TYPE CGXAsn1Converter::GetCertificateType(CGXByteBuffer &data, CGXAsn1Sequence *seq) {
...
int CGXAsn1Converter::EncodeSubject(std::string &value, CGXAsn1Sequence *list) {
    DLMS_X509_NAME name;
    CGXAsn1Base *val = NULL;
    std::vector<std::string> arr = GXHelpers::Split(value, ',');
    for (std::vector<std::string>::iterator str = arr.begin(); str != arr.end(); ++str) {
        ...
        CGXAsn1ObjectIdentifier *key = new CGXAsn1ObjectIdentifier(oid);
        list->GetValues()->push_back(new CGXAsn1Set(key, val));
    }
    return 0;
}
...
int CGXAsn1Converter::FromByteArray(CGXByteBuffer &data, CGXAsn1Base *&value) {
    int ret;
...
int CGXAsn1Converter::GetNext(CGXByteBuffer &data, CGXDLMSVariant &value) {
    int ret;
    std::vector<CGXAsn1Base *> objects;
...
int CGXAsn1Converter::ToByteArray(CGXAsn1Base *objects, CGXByteBuffer &value) {
    int count = 0;
    return GetBytes(value, objects, count);
}

std::string CGXAsn1Converter::SystemTitleToSubject(CGXByteBuffer &systemTitle) {
    return "CN=" + systemTitle.ToHexString(false);
}

int CGXAsn1Converter::SystemTitleFromSubject(std::string &subject, CGXByteBuffer &value) {
    std::string tmp;
...
int CGXAsn1Converter::HexSystemTitleFromSubject(std::string &subject, std::string &value) {
...
DLMS_PKCS_TYPE CGXAsn1Converter::GetCertificateType(std::string &der) {
    CGXByteBuffer bb;
    bb.FromBase64(der);
