 ...
 */
int GetAuthenticationString(CGXDLMSSettings &settings, CGXByteBuffer &data, bool ignoreAcse) {
    if (settings.GetAuthentication() != DLMS_AUTHENTICATION_NONE ||
        ...
        data.SetUInt8(BER_TYPE_CONTEXT | PDU_TYPE_SENDER_ACSE_REQUIREMENTS);
        data.SetUInt8(2);
        data.SetUInt8(BER_TYPE_BIT_STRING | BER_TYPE_OCTET_STRING);
        data.SetUInt8(0x80);
...
int CGXAPDU::GenerateApplicationContextName(CGXDLMSSettings &settings, CGXByteBuffer &data, CGXCipher *cipher) {
...
int GetConformanceFromArray(CGXByteBuffer &data, unsigned int &value) {
    ...
    if ((ret = data.GetUInt8(&v1)) == 0 && (ret = data.GetUInt8(&v2)) == 0 && (ret = data.GetUInt8(&v3)) == 0) {
    ...
    if (response) {
        ...
        if (xml != NULL) {
            if (initiateRequest || xml->GetOutputType() == DLMS_TRANSLATOR_OUTPUT_TYPE_SIMPLE_XML) {
                xml->IntegerToHex((int32_t)vaa, 4, str);
...
int CGXAPDU::ParseInitiate(
    ...
    return Parse(
        initiateRequest, settings, cipher, data,
#ifndef DLMS_IGNORE_XML_TRANSLATOR
        xml,
#endif  //DLMS_IGNORE_XML_TRANSLATOR
        tag
...
#endif  //DLMS_IGNORE_XML_TRANSLATOR
    return ParseInitiate(
        false, settings, cipher, data
#ifndef DLMS_IGNORE_XML_TRANSLATOR
        ,
        xml
#endif  //DLMS_IGNORE_XML_TRANSLATOR
...
int ValidateAare(CGXDLMSSettings &settings, CGXByteBuffer &buff) {
    int ret;
    unsigned char tag;
    ...
    if (settings.IsServer()) {
        if (tag != (BER_TYPE_APPLICATION | BER_TYPE_CONSTRUCTED | PDU_TYPE_PROTOCOL_VERSION)) {
            return DLMS_ERROR_CODE_INVALID_TAG;
        }
    } else {
        if (tag != (BER_TYPE_APPLICATION | BER_TYPE_CONSTRUCTED | PDU_TYPE_APPLICATION_CONTEXT_NAME)) {
            return DLMS_ERROR_CODE_INVALID_TAG;
        }
    }
    return 0;
    ...
    if (xml != NULL) {
        if (xml->GetOutputType() == DLMS_TRANSLATOR_OUTPUT_TYPE_SIMPLE_XML) {
            ...
            if (settings.GetAuthentication() == DLMS_AUTHENTICATION_LOW) {
                ...
                xml->AppendLine(TRANSLATOR_GENERAL_TAGS_CALLING_AUTHENTICATION, "", str);
            } else {
...
int UpdateAuthentication(CGXDLMSSettings &settings, CGXByteBuffer &buff) {
    int ret;
    unsigned char ch;
    if ((ret = buff.GetUInt8(&ch)) != 0) {
        return ret;
...
int CGXAPDU::GetUserInformation(CGXDLMSSettings &settings, CGXCipher *cipher, CGXByteBuffer &data) {
    data.Clear();
...
int ParseProtocolVersion(CGXDLMSSettings &settings, CGXByteBuffer &buff) {
...
#ifndef DLMS_IGNORE_XML_TRANSLATOR
void AppendServerSystemTitleToXml(CGXDLMSSettings &settings, CGXDLMSTranslatorStructure *xml, int tag) {
    if (xml != NULL) {
        std::string str;
    ...
    while (buff.GetPosition() < buff.GetSize()) {
        ...
        switch (tag) {
            ...
            case BER_TYPE_CONTEXT | BER_TYPE_CONSTRUCTED | PDU_TYPE_APPLICATION_CONTEXT_NAME: {
                if ((ret = ParseApplicationContextName(
                         settings, buff
#ifndef DLMS_IGNORE_XML_TRANSLATOR
                         ,
                         xml
                ...
                && diagnostic != DLMS_SOURCE_DIAGNOSTIC_NONE)
            {
                ...
                if ((ret = ParseUserInformation(
                         settings, cipher, buff
#ifndef DLMS_IGNORE_XML_TRANSLATOR
                         ,
                         xml
#endif  //DLMS_IGNORE_XML_TRANSLATOR
