...
int CGXPkcs10::Init(CGXByteBuffer &data) {
	m_RawData = data;
											...
											if (CGXAsn1Sequence *tmp4 = dynamic_cast<CGXAsn1Sequence *>(tmp3)) {
												if (CGXAsn1Variant *tmp5 = dynamic_cast<CGXAsn1Variant *>(tmp4->GetValues()->at(0))) {
													...
													if (CGXAsn1Variant *tmp6 = dynamic_cast<CGXAsn1Variant *>(tmp4->GetValues()->at(1))) {
														...
														data.SetPosition(0);
														bool verify;
														if (e.Verify(bb, bb2, verify) != 0 || verify != true) {
#ifdef _DEBUG
...
int CGXPkcs10::FromHexString(std::string &data, CGXPkcs10 &cert) {
	CGXByteBuffer bb;
	GXHelpers::HexToBytes(data, bb);
	return cert.Init(bb);
}

int CGXPkcs10::FromByteArray(const unsigned char *data, uint16_t length, CGXPkcs10 &cert) {
	CGXByteBuffer bb;
	bb.Set(data, length);
	return cert.Init(bb);
}

int CGXPkcs10::FromByteArray(CGXByteBuffer &data, CGXPkcs10 &cert) {
	return cert.Init(data);
}

int CGXPkcs10::FromPem(std::string data, CGXPkcs10 &cert) {
...
int CGXPkcs10::FromDer(std::string &data, CGXPkcs10 &cert) {
	int ret;
	CGXByteBuffer bb;
...
int CGXPkcs10::FromHexString(CGXByteBuffer &data, CGXPkcs10 &cert) {
	return cert.Init(data);
}
...
int CGXPkcs10::Sign(CGXPrivateKey &key, DLMS_HASH_ALGORITHM hashAlgorithm) {
	CGXByteBuffer data;
	CGXAsn1Sequence s;
	int ret = GetData(&s);
	if (ret == 0) {
	...
	for (std::vector<CGXCertificateRequest>::iterator it = certifications.begin(); it != certifications.end(); ++it) {
		...
		switch (it->GetCertificateType()) {
			case DLMS_CERTIFICATE_TYPE_DIGITAL_SIGNATURE:
				usage.append(std::to_string(DLMS_KEY_USAGE_DIGITAL_SIGNATURE));
				break;
			...
			case DLMS_CERTIFICATE_TYPE_TLS:
				usage.append(std::to_string(DLMS_KEY_USAGE_DIGITAL_SIGNATURE | DLMS_KEY_USAGE_KEY_AGREEMENT));
				break;
			default:
#ifdef _DEBUG
				printf("Invalid type.");
...
int CGXPkcs10::Load(std::string &path, CGXPkcs10 &cert) {
	std::string text;
...
int CGXPkcs10::Load(const char *path, CGXPkcs10 &cert) {
	std::string tmp = path;
...
bool CGXPkcs10::Equals(CGXPkcs10 &cert) {
	return m_RawData.Compare(cert.m_RawData.GetData(), cert.m_RawData.GetSize());
}