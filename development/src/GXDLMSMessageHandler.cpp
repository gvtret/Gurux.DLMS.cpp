...
//Constructor.
CGXDLMSMessageHandler::CGXDLMSMessageHandler(): CGXDLMSMessageHandler("", 0) {
}

//SN Constructor.
CGXDLMSMessageHandler::CGXDLMSMessageHandler(std::string ln, unsigned short sn): CGXDLMSObject(DLMS_OBJECT_TYPE_MESSAGE_HANDLER, ln, sn) {
}

//LN Constructor.
CGXDLMSMessageHandler::CGXDLMSMessageHandler(std::string ln): CGXDLMSMessageHandler(ln, 0) {
}
...
void CGXDLMSMessageHandler::GetValues(std::vector<std::string> &values) {
    ...
    sb << '[';
    empty = true;
    for (std::vector<std::pair<std::string, std::pair<int, CGXDLMSScriptAction>>>::iterator it = m_SendersAndActions.begin(); it != m_SendersAndActions.end();
         ++it) {
        if (!empty) {
            sb << ", ";
        }
