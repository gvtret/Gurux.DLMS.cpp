...
bool CGXDLMS::UseHdlc(DLMS_INTERFACE_TYPE type) {
    return type == DLMS_INTERFACE_TYPE_HDLC || type == DLMS_INTERFACE_TYPE_HDLC_WITH_MODE_E || type == DLMS_INTERFACE_TYPE_PLC_HDLC;
}

bool CGXDLMS::IsReplyMessage(DLMS_COMMAND cmd) {
    return cmd == DLMS_COMMAND_GET_RESPONSE || cmd == DLMS_COMMAND_SET_RESPONSE || cmd == DLMS_COMMAND_METHOD_RESPONSE;
}
...
int CGXDLMS::GetAddress(long value, unsigned long &address, int &size) {
    if (value < 0x80) {
        ...
        address = (unsigned long)((value & 0xFE00000) << 4 | (value & 0x1FC000) << 3 | (value & 0x3F80) << 2 | (value & 0x7F) << 1 | 1);
        size = 4;
    } else {
        //Invalid address
...
int CGXDLMS::ReceiverReady(CGXDLMSSettings &settings, DLMS_DATA_REQUEST_TYPES type, CGXCipher *cipher, CGXByteBuffer &reply) {
    CGXReplyData data;
...
int CGXDLMS::ReceiverReady(CGXDLMSSettings &settings, CGXReplyData &data, CGXCipher *cipher, CGXByteBuffer &reply) {
    ...
    if (data.GetMoreData() == DLMS_DATA_REQUEST_TYPES_GBT) {
        CGXDLMSLNParameters p(&settings, 0, DLMS_COMMAND_GENERAL_BLOCK_TRANSFER, 0, NULL, NULL, 0xff, DLMS_COMMAND_NONE);
        ...
        if (settings.GetUseLogicalNameReferencing()) {
            CGXDLMSLNParameters p(&settings, 0, cmd, DLMS_GET_COMMAND_TYPE_NEXT_DATA_BLOCK, &bb, NULL, 0xff, DLMS_COMMAND_NONE);
            ret = GetLnMessages(p, tmp);
        } else {
            CGXDLMSSNParameters p(&settings, cmd, 1, DLMS_VARIABLE_ACCESS_SPECIFICATION_BLOCK_NUMBER_ACCESS, &bb, NULL);
...
*/
int CGXDLMS::GetWrapperFrame(CGXDLMSSettings &settings, DLMS_COMMAND command, CGXByteBuffer &data, CGXByteBuffer &reply) {
    reply.Clear();
    // Add version.
    reply.SetUInt16(1);
...
int CGXDLMS::GetHdlcFrame(CGXDLMSSettings &settings, unsigned char frame, CGXByteBuffer *data, CGXByteBuffer &reply) {
    reply.Clear();
    ...
    if (data == NULL || data->GetSize() == 0) {
        ...
        len = data->Available();
        // Is last packet.
        reply.SetUInt8(0xA0 | (((7 + primaryAddress.GetSize() + secondaryAddress.GetSize() + len) >> 8) & 0x7));
    } else {
        len = frameSize;
        // More data to left.
    ...
    // Frame len.
    if (len == 0) {
        reply.SetUInt8((unsigned char)(5 + primaryAddress.GetSize() + secondaryAddress.GetSize() + len));
    } else {
        reply.SetUInt8((unsigned char)(7 + primaryAddress.GetSize() + secondaryAddress.GetSize() + len));
    }
    // Add primary address.
...
int CGXDLMS::GetMacFrame(CGXDLMSSettings &settings, unsigned char frame, unsigned char creditFields, CGXByteBuffer *data, CGXByteBuffer &reply) {
...
int CGXDLMS::GetPlcFrame(CGXDLMSSettings &settings, unsigned char creditFields, CGXByteBuffer *data, CGXByteBuffer &reply) {
...
int CGXDLMS::GetMacHdlcFrame(CGXDLMSSettings &settings, unsigned char frame, unsigned char creditFields, CGXByteBuffer *data, CGXByteBuffer &reply) {
...
unsigned char IsGloMessage(unsigned char cmd) {
    return cmd == DLMS_COMMAND_GLO_GET_REQUEST || cmd == DLMS_COMMAND_GLO_SET_REQUEST || cmd == DLMS_COMMAND_GLO_METHOD_REQUEST;
}

int Cipher0(CGXDLMSLNParameters &p, CGXByteBuffer &reply) {
    int ret;
    CGXByteBuffer tmp;
    CGXByteBuffer *key;
    unsigned char cmd;
    ...
    if (p.GetCipheredCommand() == DLMS_COMMAND_NONE) {
        if (((p.GetSettings()->GetConnected() & DLMS_CONNECTION_STATE_DLMS) == 0 ||
            ...
            (p.GetSettings()->GetPreEstablishedSystemTitle().GetSize() == 0 ||
             (p.GetSettings()->GetProposedConformance() & DLMS_CONFORMANCE_GENERAL_PROTECTION) == 0)) {
            if (p.GetSettings()->GetCipher()->GetDedicatedKey().GetSize() != 0 && (p.GetSettings()->GetConnected() & DLMS_CONNECTION_STATE_DLMS) != 0) {
    ...
    ret = p.GetSettings()->GetCipher()->Encrypt(
        p.GetSettings()->GetCipher()->GetSecuritySuite(), p.GetSettings()->GetCipher()->GetSecurity(), DLMS_COUNT_TYPE_PACKET,
        p.GetSettings()->GetCipher()->GetFrameCounter(), cmd, title, *key, reply, true
    );
    if (ret != 0) {
        return ret;
    }
...
int CGXDLMS::GetLNPdu(CGXDLMSLNParameters &p, CGXByteBuffer &reply) {
    int ret;
    unsigned char ciphering = p.GetCommand() != DLMS_COMMAND_AARQ && p.GetCommand() != DLMS_COMMAND_AARE && p.GetSettings()->GetCipher() != NULL &&
    ...
    if (p.GetCommand() == DLMS_COMMAND_AARQ) {
        ...
        if (p.GetCommand() == DLMS_COMMAND_EVENT_NOTIFICATION || p.GetCommand() == DLMS_COMMAND_DATA_NOTIFICATION ||
            p.GetCommand() == DLMS_COMMAND_ACCESS_REQUEST || p.GetCommand() == DLMS_COMMAND_ACCESS_RESPONSE) {
            ...
            if (p.GetCommand() != DLMS_COMMAND_GET_REQUEST && p.GetData() != NULL && p.GetData()->GetSize() != 0) {
        ...
        if (len == 0) {
            ...
            if (p.GetData() != NULL && p.GetData()->GetSize() != 0) {
                ...
                // Get request size can be bigger than PDU size.
                if (p.GetCommand() != DLMS_COMMAND_GET_REQUEST && len + reply.GetSize() > p.GetSettings()->GetMaxPduSize()) {
                    len = p.GetSettings()->GetMaxPduSize() - reply.GetSize() - p.GetData()->GetPosition();
                }
                reply.Set(p.GetData(), p.GetData()->GetPosition(), len);
            }
        }
...
int CGXDLMS::GetLnMessages(CGXDLMSLNParameters &p, std::vector<CGXByteBuffer> &messages) {
    int ret;
    messages.clear();
    CGXByteBuffer reply, tmp;
    unsigned char frame = 0;
    if (p.GetCommand() == DLMS_COMMAND_DATA_NOTIFICATION || p.GetCommand() == DLMS_COMMAND_EVENT_NOTIFICATION) {
        frame = 0x13;
    }
    do {
...
int CGXDLMS::AppendMultipleSNBlocks(CGXDLMSSNParameters &p, CGXByteBuffer &reply) {
    ...
    // Add LLC bytes.
    if (p.GetCommand() == DLMS_COMMAND_WRITE_REQUEST || p.GetCommand() == DLMS_COMMAND_READ_REQUEST) {
...
int CGXDLMS::GetSNPdu(CGXDLMSSNParameters &p, CGXByteBuffer &reply) {
    int ret;
    unsigned char ciphering = p.GetCommand() != DLMS_COMMAND_AARQ && p.GetCommand() != DLMS_COMMAND_AARE && p.GetSettings()->GetCipher() != NULL &&
        p.GetSettings()->GetCipher()->GetSecurity() != DLMS_SECURITY_NONE;
    ...
    if (p.GetCommand() == DLMS_COMMAND_INFORMATION_REPORT) {
        ...
        if (!p.IsMultipleBlocks()) {
            ...
            if (p.IsMultipleBlocks()) {
                ...
                if (p.GetCommand() == DLMS_COMMAND_WRITE_REQUEST) {
                    p.SetRequestType(DLMS_VARIABLE_ACCESS_SPECIFICATION_WRITE_DATA_BLOCK_ACCESS);
                } else if (p.GetCommand() == DLMS_COMMAND_READ_REQUEST) {
    ...
    if (ciphering && p.GetCommand() != DLMS_COMMAND_AARQ && p.GetCommand() != DLMS_COMMAND_AARE) {
        CGXByteBuffer tmp;
        ret = p.GetSettings()->GetCipher()->Encrypt(
            p.GetSettings()->GetCipher()->GetSecuritySuite(), p.GetSettings()->GetCipher()->GetSecurity(), DLMS_COUNT_TYPE_PACKET,
            p.GetSettings()->GetCipher()->GetFrameCounter(), GetGloMessage(p.GetCommand()), p.GetSettings()->GetCipher()->GetSystemTitle(),
            p.GetSettings()->GetCipher()->GetAuthenticationKey(), reply, true
        );
        if (ret != 0) {
            return ret;
        }
...
int CGXDLMS::GetSnMessages(CGXDLMSSNParameters &p, std::vector<CGXByteBuffer> &messages) {
    int ret;
    CGXByteBuffer data, reply;
    unsigned char frame = 0x0;
    if (p.GetCommand() == DLMS_COMMAND_INFORMATION_REPORT || p.GetCommand() == DLMS_COMMAND_DATA_NOTIFICATION) {
        frame = 0x13;
    }
    do {
        ret = GetSNPdu(p, data);
...
int CGXDLMS::GetHdlcData(bool server, CGXDLMSSettings &settings, CGXByteBuffer &reply, CGXReplyData &data, unsigned char &frame, CGXReplyData *notify) {
    ...
    if (frame != 0x3 && frame != 0x13 && (frame & HDLC_FRAME_TYPE_U_FRAME) == HDLC_FRAME_TYPE_U_FRAME) {
        ...
        if (tmp == HDLC_CONTROL_FRAME_REJECT) {
            ...
            if (settings.GetCommand() == DLMS_COMMAND_GET_REQUEST || settings.GetCommand() == DLMS_COMMAND_GLO_GET_REQUEST) {
...
int CGXDLMS::GetHDLCAddress(CGXByteBuffer &buff, unsigned long &address) {
    unsigned char ch;
    unsigned short s;
    uint32_t l;
    int ret, size = 0;
...
int CGXDLMS::CheckHdlcAddress(bool server, CGXDLMSSettings &settings, CGXByteBuffer &reply, int index, unsigned long &source, unsigned long &target) {
    unsigned char ch;
    int ret;
...
int HandleActionResponseNormal(CGXDLMSSettings &settings, CGXReplyData &data) {
    int ret;
...
int HandleActionResponseWithBlock(CGXDLMSSettings &settings, CGXReplyData &reply, unsigned long index) {
    int ret = 0;
    unsigned char ch;
    uint32_t number;
    std::string str;
...
int CGXDLMS::HandleMethodResponse(CGXDLMSSettings &settings, CGXReplyData &data, unsigned long index) {
    int ret;
...
int CGXDLMS::HandleAccessResponse(CGXDLMSSettings &settings, CGXReplyData &reply) {
    int ret;
    std::string str;
    unsigned char ch;
    uint32_t invokeId;
...
int CGXDLMS::HandleSetResponse(CGXDLMSSettings &settings, CGXReplyData &data) {
    std::string str;
...
int CGXDLMS::HandleGbt(CGXDLMSSettings &settings, CGXReplyData &data) {
    ...
    if ((bc & 0x80) == 0) {
        ...
        if (data.GetData().GetPosition() != data.GetData().GetSize() &&
            (data.GetCommand() == DLMS_COMMAND_READ_RESPONSE || data.GetCommand() == DLMS_COMMAND_GET_RESPONSE) &&
...
int CGXDLMS::HandleGloDedRequest(CGXDLMSSettings &settings, CGXReplyData &data) {
        ...
        if ((data.GetMoreData() & DLMS_DATA_REQUEST_TYPES_FRAME) == 0) {
            ...
            if (settings.GetCipher()->GetDedicatedKey().GetSize() != 0 && (settings.GetConnected() & DLMS_CONNECTION_STATE_DLMS) != 0) {
                if ((ret = settings.GetCipher()->Decrypt(
                         settings.GetSourceSystemTitle(), settings.GetCipher()->GetDedicatedKey(), data.GetData(), security, suite, InvocationCounter
                     )) != 0) {
                    return ret;
                }
            }
            //If pre-set connection is made.
                ...
                if ((ret = settings.GetCipher()->Decrypt(
                         settings.GetSourceSystemTitle(), settings.GetCipher()->GetBlockCipherKey(), data.GetData(), security, suite, InvocationCounter
                     )) != 0) {
                    return ret;
                }
            }
            ...
            if (data.GetCommand() == DLMS_COMMAND_DATA_NOTIFICATION || data.GetCommand() == DLMS_COMMAND_INFORMATION_REPORT) {
                data.SetCommand(DLMS_COMMAND_NONE);
                data.GetData().SetPosition(data.GetData().GetPosition() - 1);
                GetPdu(settings, data);
            }
...
int CGXDLMS::HandleGloDedResponse(CGXDLMSSettings &settings, CGXReplyData &data, int index) {
    if (settings.GetCipher() == NULL) {
        //Secure connection is not supported.
        return DLMS_ERROR_CODE_INVALID_PARAMETER;
    }
...
#endif  //DLMS_IGNORE_XML_TRANSLATOR
    {
        ...
        if ((data.GetMoreData() & DLMS_DATA_REQUEST_TYPES_FRAME) == 0) {
            ...
            if ((ret = settings.GetCipher()->Decrypt(settings.GetSourceSystemTitle(), *key, bb, security, suite, invocationCounter)) != 0) {
                return ret;
            }
...
int CGXDLMS::HandleGeneralCiphering(CGXDLMSSettings &settings, CGXReplyData &data) {
    unsigned char ch;
    int ret;
    if (settings.GetCipher() == NULL) {
        //Secure connection is not supported.
...
int CGXDLMS::GetPdu(CGXDLMSSettings &settings, CGXReplyData &data) {
    ...
    if (cmd == DLMS_COMMAND_NONE) {
        ...
        if (!data.GetPeek() && data.GetMoreData() == DLMS_DATA_REQUEST_TYPES_NONE) {
            if (data.GetCommand() == DLMS_COMMAND_AARE || data.GetCommand() == DLMS_COMMAND_AARQ) {
                data.GetData().SetPosition(0);
            } else {
    ...
    if (cmd == DLMS_COMMAND_READ_RESPONSE && data.GetCommandType() == DLMS_SINGLE_READ_RESPONSE_DATA_BLOCK_RESULT &&
        (data.GetMoreData() & DLMS_DATA_REQUEST_TYPES_FRAME) != 0) {
        return 0;
    }
...
int CGXDLMS::GetData(CGXDLMSSettings &settings, CGXByteBuffer &reply, CGXReplyData &data, CGXReplyData *notify) {
...
int CGXDLMS::HandleGetResponseWithList(CGXDLMSSettings &settings, CGXReplyData &reply) {
    int ret;
    unsigned char ch;
    uint32_t count;
...
int CGXDLMS::HandleGetResponseNormal(CGXDLMSSettings &settings, CGXReplyData &reply, bool &empty) {
    int ret = 0;
    unsigned char ch;
    CGXByteBuffer &data = reply.GetData();
    if (data.Available() == 0) {
...
int CGXDLMS::HandleGetResponseNextDataBlock(CGXDLMSSettings &settings, CGXReplyData &reply, int index) {
    int ret = 0;
    uint32_t number;
    uint32_t count;
    unsigned char ch;
    ...
    if (reply.GetMoreData() == DLMS_DATA_REQUEST_TYPES_NONE && settings.GetCommand() == DLMS_COMMAND_GET_REQUEST &&
...
int CGXDLMS::HandleGetResponse(CGXDLMSSettings &settings, CGXReplyData &reply, int index) {
    int ret;
    unsigned char ch;
    short type;
#ifndef DLMS_IGNORE_XML_TRANSLATOR
...
int CGXDLMS::ReadResponseDataBlockResult(CGXDLMSSettings &settings, CGXReplyData &reply, int index) {
    int ret;
...
int CGXDLMS::HandleReadResponse(CGXDLMSSettings &settings, CGXReplyData &reply, int index) {
    std::string str;
    unsigned char ch;
    ...
    for (pos = 0; pos != cnt; ++pos) {
        ...
        switch (type) {
            ...
            case DLMS_SINGLE_READ_RESPONSE_DATA_ACCESS_ERROR:
                ...
                CGXDLMSTranslator::ErrorCodeToString(reply.GetXml()->GetOutputType(), (DLMS_ERROR_CODE)ch, str);
                reply.GetXml()->AppendLine(DLMS_COMMAND_READ_RESPONSE << 8 | DLMS_SINGLE_READ_RESPONSE_DATA_ACCESS_ERROR, "", str);
                if (standardXml) {
                    reply.GetXml()->AppendEndTag(DLMS_TRANSLATOR_TAGS_CHOICE);
                }
#else
...
int CGXDLMS::GetTcpData(CGXDLMSSettings &settings, CGXByteBuffer &buff, CGXReplyData &data, CGXReplyData *notify) {
    CGXReplyData *target = &data;
    int ret;
    // If whole frame is not received yet.
...
int CGXDLMS::GetValueFromData(CGXDLMSSettings &settings, CGXReplyData &reply) {
    ...
    } else if (info.IsComplete() && reply.GetCommand() == DLMS_COMMAND_DATA_NOTIFICATION) {
    ...
    if (reply.GetCommand() != DLMS_COMMAND_DATA_NOTIFICATION && info.IsComplete() && reply.GetMoreData() == DLMS_DATA_REQUEST_TYPES_NONE) {
...
int CGXDLMS::GetMBusData(CGXDLMSSettings &settings, CGXByteBuffer &buff, CGXReplyData &data) {
    int ret;
...
int CGXDLMS::GetPlcData(CGXDLMSSettings &settings, CGXByteBuffer &buff, CGXReplyData &data) {
    if (buff.Available() < 9) {
        data.SetComplete(false);
        return 0;
    }
    ...
    if (buff.Available() < len) {
        ...
        if (buff.GetSize() < (unsigned short)(len + padLen + 2)) {
            ...
            unsigned char da, sa;
            if ((ret = buff.GetUInt8(&da)) != 0 || (ret = buff.GetUInt8(&sa)) != 0) {
                return ret;
            }
            if (settings.IsServer()) {
                data.SetComplete(
                ...
                data.SetComplete(
                    ...
                    data.GetXml() != NULL ||
#endif  //DLMS_IGNORE_XML_TRANSLATOR
                    (macDa == DLMS_PLC_DESTINATION_ADDRESS_ALL_PHYSICAL || macDa == DLMS_PLC_SOURCE_ADDRESS_INITIATOR ||
...
int CGXDLMS::GetPlcHdlcData(CGXDLMSSettings &settings, CGXByteBuffer &buff, CGXReplyData &data, unsigned char *frame) {
    if (buff.Available() < 2) {
        data.SetComplete(false);
        return 0;
    }
...
int CGXDLMS::CheckWrapperAddress(CGXDLMSSettings &settings, CGXByteBuffer &buff, CGXReplyData *notify) {
    int ret;
    unsigned short value;
    if (settings.IsServer()) {
        if ((ret = buff.GetUInt16(&value)) != 0) {
...
int CGXDLMS::AppendData(CGXDLMSSettings *settings, CGXDLMSObject *obj, unsigned char index, CGXByteBuffer &bb, CGXDLMSVariant &value) {
    int ret;
    DLMS_DATA_TYPE tp;
...
int CGXDLMS::ParseSnrmUaResponse(CGXByteBuffer &data, CGXHdlcSettings *limits) {
