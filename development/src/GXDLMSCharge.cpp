...
//Constructor.
CGXDLMSCharge::CGXDLMSCharge(): CGXDLMSCharge("0.0.19.20.0.255", 0) {
}

//SN Constructor.
CGXDLMSCharge::CGXDLMSCharge(std::string ln, unsigned short sn): CGXDLMSObject(DLMS_OBJECT_TYPE_CHARGE, ln, sn) {
    m_ChargeType = DLMS_CHARGE_TYPE_CONSUMPTION_BASED_COLLECTION;
}

//LN Constructor.
CGXDLMSCharge::CGXDLMSCharge(std::string ln): CGXDLMSCharge(ln, 0) {
}
...
int CGXDLMSCharge::GetUnitCharge(CGXUnitCharge &charge, CGXByteBuffer &bb) {
    ...
    for (std::vector<CGXChargeTable>::iterator it = charge.GetChargeTables().begin();
         it != charge.GetChargeTables().end(); ++it) {
        bb.SetUInt8(DLMS_DATA_TYPE_STRUCTURE);
        bb.SetUInt8(2);
        tmp = it->GetIndex();
