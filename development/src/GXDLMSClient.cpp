...
int CGXDLMSClient::ParseSNObjectItem(CGXDLMSVariant &value, bool ignoreInactiveObjects) {
...
int CGXDLMSClient::ParseLNObjectItem(CGXDLMSVariant &value, bool ignoreInactiveObjects) {
    int ret;
    if (value.Arr.size() != 4) {
        ...
        if (pObj != NULL) {
            ...
            if (value.Arr[3].Arr.size() == 2) {
                ...
                for (unsigned int pos = 0; pos != value.Arr[3].Arr[1].Arr.size(); ++pos) {
                    ...
                    if (tmp.vt != DLMS_DATA_TYPE_STRUCTURE || tmp.Arr.size() != 2) {
                        delete pObj;
                        return DLMS_ERROR_CODE_INVALID_PARAMETER;
                    }
...
int CGXDLMSClient::ParseAAREResponse(CGXByteBuffer &reply) {
    ...
    if ((ret = CGXAPDU::ParsePDU(m_Settings, m_Settings.GetCipher(), reply, result, sd, NULL)) != 0) {
        return ret;
    }
#else
    if ((ret = CGXAPDU::ParsePDU(m_Settings, m_Settings.GetCipher(), reply, result, sd)) != 0) {
        return ret;
    }
...
int CGXDLMSClient::GetApplicationAssociationRequest(std::vector<CGXByteBuffer> &packets) {
    int ret;
    ...
    if (m_Settings.GetAuthentication() == DLMS_AUTHENTICATION_HIGH_GMAC) {
        ...
        pw.Set(m_Settings.GetPassword().GetData(), m_Settings.GetPassword().GetSize());
        pw.Set(c->GetSystemTitle().GetData(), c->GetSystemTitle().GetSize());
        pw.Set(m_Settings.GetSourceSystemTitle().GetData(), m_Settings.GetSourceSystemTitle().GetSize());
        pw.Set(m_Settings.GetStoCChallenge().GetData(), m_Settings.GetStoCChallenge().GetSize());
        pw.Set(m_Settings.GetCtoSChallenge().GetData(), m_Settings.GetCtoSChallenge().GetSize());
    } else if (m_Settings.GetAuthentication() == DLMS_AUTHENTICATION_HIGH_ECDSA) {
        std::pair<CGXPublicKey, CGXPrivateKey> &kp = c->GetSigningKeyPair();
        if (kp.first.GetRawValue().GetSize() == 0 || kp.second.GetRawValue().GetSize() == 0) {
#ifdef _DEBUG
...
int CGXDLMSClient::ParseApplicationAssociationResponse(CGXByteBuffer &reply) {
    ...
    if (memcmp(m_ManufacturerId, "LGZ", 3) == 0 && m_Settings.GetAuthentication() == DLMS_AUTHENTICATION_HIGH) {
        ...
        if (value.vt != DLMS_DATA_TYPE_NONE) {
            if (m_Settings.GetAuthentication() == DLMS_AUTHENTICATION_HIGH_ECDSA) {
                ...
                if ((ret = CGXSecure::Secure(m_Settings, m_Settings.GetCipher(), ic, cToS, secret, challenge)) != 0) {
                    return ret;
                }
...
int CGXDLMSClient::ReceiverReady(CGXReplyData &reply, CGXByteBuffer &packets) {
...
int CGXDLMSClient::Read(
    ...
    if (GetUseLogicalNameReferencing()) {
        ...
        CGXDLMSSNParameters p(&m_Settings, DLMS_COMMAND_READ_REQUEST, 1, requestType, &attributeDescriptor, parameters);
...
int CGXDLMSClient::ReadList(
    ...
    if (GetUseLogicalNameReferencing()) {
        ...
        CGXDLMSSNParameters p(&m_Settings, DLMS_COMMAND_READ_REQUEST, (unsigned long)list.size(), 0xFF, &bb, NULL);
        ret = CGXDLMS::GetSnMessages(p, reply);
    }
    return ret;
}
...
int CGXDLMSClient::WriteList(
    ...
    if (GetUseLogicalNameReferencing()) {
        ...
        if (ret == 0) {
            CGXDLMSSNParameters p(&m_Settings, DLMS_COMMAND_WRITE_REQUEST, (unsigned long)list.size(), 0xFF, &bb, NULL);
            ret = CGXDLMS::GetSnMessages(p, reply);
        }
    }
    return ret;
...
int CGXDLMSClient::Write(CGXDLMSObject *pObject, int index, std::vector<CGXByteBuffer> &reply) {
    if (index < 1) {
        //Invalid parameter
        return DLMS_ERROR_CODE_INVALID_PARAMETER;
    }
...
int CGXDLMSClient::Write(CGXDLMSObject *pObject, int index, CGXDLMSVariant &data, std::vector<CGXByteBuffer> &reply) {
    CGXDLMSVariant name = pObject->GetName();
...
int CGXDLMSClient::Method(
    ...
    if (GetUseLogicalNameReferencing()) {
        ...
        CGXDLMSSNParameters p(&m_Settings, DLMS_COMMAND_READ_REQUEST, 1, requestType, &bb, &data);
        ret = CGXDLMS::GetSnMessages(p, reply);
    }
...
int CGXDLMSClient::Method(
    ...
    if (GetUseLogicalNameReferencing()) {
        ...
        // Method Invocation Parameters is not used.
        bb.SetUInt8(1);
        CGXDLMSSNParameters p(&m_Settings, DLMS_COMMAND_READ_REQUEST, 1, requestType, &bb, &value);
        ret = CGXDLMS::GetSnMessages(p, reply);
    }
    return ret;
}

int CGXDLMSClient::ReadRowsByEntry(CGXDLMSProfileGeneric *pg, int index, int count, std::vector<CGXByteBuffer> &reply) {
