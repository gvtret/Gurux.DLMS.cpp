...
/**
    ...
    */
int CGXDLMSServer::HandleAarqRequest(CGXByteBuffer &data, CGXDLMSConnectionEventArgs &connectionInfo) {
    int ret;
    CGXByteBuffer error;
...
int CGXDLMSServer::HandleGeneralBlockTransfer(CGXServerReply &sr, CGXByteBuffer &data, unsigned char cipheredCommand) {
    unsigned char bc;
    ...
    if (m_Transaction != NULL) {
        if (m_Transaction->GetCommand() == DLMS_COMMAND_GET_REQUEST ||
            ...
            if ((bc & 0x80) != 0) {
                HandleCommand(m_Transaction->GetCommand(), m_Transaction->GetData(), sr, cipheredCommand);
                delete m_Transaction;
                m_Transaction = NULL;
                igonoreAck = false;
                windowSize = 1;
...
int CGXDLMSServer::HandleRequest(CGXServerReply &sr) {
    ...
    CGXByteBuffer &reply = sr.GetReply();
    bool first = m_Settings.GetServerAddress() == 0 && m_Settings.GetClientAddress() == 0;
...
int CGXDLMSServer::AddData(CGXDLMSObject *obj, unsigned char index, CGXByteBuffer &buff) {
    int ret;
    DLMS_DATA_TYPE dt;
